TAG.Util.makeNamespace("TAG.Layout.Exhibitions");

TAG.Layout.Exhibitions = function (options, exhibitionCallback) {
    "use strict";

    options = TAG.Util.setToDefaults(options, TAG.Layout.Exhibitions.default_options);
    var root, leftbar, displayarea, exhibitarea, currentfolder, currentExhElements, leftbarHeader, exhibitionLabel, toursLabel, selectText, exhibitionSpan, toursSpan, loadingArea,
        titlediv = null, timelineDiv,
 timelineDivContainer = null, currentImage = null,
        sub1 = null,
        sub2 = null,
    ...
    */
    function init() {
        ...
        var circle = TAG.Util.showProgressCircle(loadingArea, progressCircCSS, centerhor, centerver, false);
        ...
        backbuttonIcon.mouseleave(function () {
            TAG.Util.UI.cgBackColor("backButton", backbuttonIcon, true);
        });
        backbuttonIcon.mousedown(function () {
            TAG.Util.UI.cgBackColor("backButton", backbuttonIcon, false);
        });
        ...
        var fontSize = TAG.Util.getMaxFontSizeEM('Exhibitions', 1, $(window).width() * 0.1, 1000);
        exhibitionSpan.css({
            'font-size': fontSize,
        });
        exhibitionSpan.text("Exhibitions");
        //TAG.Util.fitText(exhibitionLabel, 0.5);
        ...
        //TAG.Util.fitText(toursLabel, 0.5);
        ...
        displayHelpTitle.text("Welcome to the " + TAG.Worktop.Database.getMuseumName());
        ...
        TAG.Worktop.Database.getExhibitions(getExhibitionsHelper, null, getExhibitionsHelper);

        /**helper function to exhibitions**/
        function getExhibitionsHelper(exhibitionsLocal) {
            TAG.Worktop.Database.getTours(getAllToursHelper, null, getAllToursHelper);
            ...
            function getAllToursHelper(tours) {
                ...
                $.each(exhibitionsLocal, function (_, e) {
                    ...
                    if (!privateState) {
                        if (!gotFirst) {
                            bgimage.css('background-image', "url(" + TAG.Worktop.Database.fixPath(e.Metadata.BackgroundImage) + ")");
                        }
                        addExhibit(e);
                        gotFirst = true;
                    }
    ...
    */
    function initFeedback() {
        ...
        var feedbackBox = TAG.Util.UI.FeedbackBox(getCurrentType, getCurrentID);
    ...
    function switchPage() {
        ...
        deepZoom = new TAG.Layout.Artmode(prevInfo, curOpts, exhibition);
        /* end nbowditch edit */

        root.css({ 'overflow-x': 'hidden' });
        TAG.Util.UI.slidePageLeftSplit(root, deepZoom.getRoot());//have the page sliding to left and change to artmode animation.
    }
    ...
    function addExhibit(exhibition) {
        ...
        var title = exhibition.Name,
            ...
            imgSrc = TAG.Worktop.Database.fixPath(exhibition.Metadata.BackgroundImage),
        ...
        var circle = TAG.Util.showProgressCircle(toAdd, progressCircCSS, '0px', '0px', false);
    ...
    function addTour(tour, allTours) {
        var title = tour.Name,
            toAddImage = $(document.createElement('img')),
            imgSrc = TAG.Worktop.Database.fixPath(tour.Metadata.Thumbnail),
            toAdd = $(document.createElement('div')),
            subInfo = $(document.createElement('div')),
    ...
    */
    ...
    function loadExhibit(exhibition, privateExhib) {
        ...
        var fontSize = TAG.Util.getMaxFontSizeEM(exhibition.Name, 1.5, w, 100);
        titlediv.css({ 'font-size': fontSize });

        //TAG.Util.fitText(titlediv, 0.7);
        ...
        img1.attr('src', TAG.Worktop.Database.fixPath(exhibition.Metadata.DescriptionImage1));
        ...
        var circle = TAG.Util.showProgressCircle(descContainer, progressCircCSS, '0px', '0px', false);
        img1.load(function () {
            TAG.Util.removeProgressCircle(circle);
        });
        // View button
        ...
        bgimage.css('background-image', "url(" + TAG.Worktop.Database.fixPath(exhibition.Metadata.BackgroundImage) + ")");
        ...
        selectExhibition.mousedown(function () {
            TAG.Util.UI.cgBackColor("forwardButton", selectExhibition);
        ...
        selectExhibition.click(function () {
            overlayOnRoot.show();
            //TAG.Util.showLoading(overlayOnRoot, '5%'); DisplayAreaSub already creates a loading circle
            TAG.Worktop.Database.getArtworksIn(exhibition.Identifier, getArtworksHelper, null, getArtworksHelper);
            ...
            function getArtworksHelper(artworks) {
                if (!artworks || !artworks[0]) {//pops up a box warning user there is no artwork in the selected exhibition
                    var noArtworksOptionBox = TAG.Util.UI.makeNoArtworksOptionBox();
        ...
        selectText.css({
            'font-size': TAG.Util.getMaxFontSizeEM("Explore this exhibition", 1, 1000, selectExhibition.height()),
            'color': 'black',
            'display': 'inline-block',
            'right': '2%',
            'padding': '2%'
        ...
        //TAG.Util.fitText(selectText, .8); //josh
        ...
        //TAG.Util.fitText(descriptiontext, 2.5);
        ...
        TAG.Util.showLoading(overlayOnRoot, '8%', '60%', '60%');
        overlayOnRoot.show();

        //get all the tours that are related
        TAG.Worktop.Database.getTours(tourHelper, null, tourHelper);
        ...
        function tourHelper(tourInfo) {
            ...
            if (tourInfo.length > 0) {
                TAG.Worktop.Database.getArtworksIn(exhibition.Identifier, exhibArtworksHelper, null, exhibArtworksHelper);
            }
            else {
                //no tour at all
    ...
    function loadTour(tour, allTours) {
        ...
        // TAG.Util.fitText(titlediv, 1); //prev 0.7
        displayareasub.append(titlediv);
        currentExhElements.title = titlediv;
        ...
        var img1 = $(document.createElement('img'));
        img1.attr('src', TAG.Worktop.Database.fixPath(tour.Metadata.Thumbnail));
        ...
        bgimage.css('background-image', "url(" + TAG.Worktop.Database.fixPath(tour.Metadata.Thumbnail) + ")");

        selectTour.off('mousedown');
        selectTour.on('mousedown', function () {
            TAG.Util.UI.cgBackColor("forwardButton", selectTour);
        ...
        selectText.css({
            'font-size': TAG.Util.getMaxFontSizeEM("Begin Tour", 1, 1000, $(window).height() * 0.03),
            'color': 'black',
            'display': 'block',
            'position': 'relative',
            'left': '5%',
        ...
        root.append(overlayOnRoot);
        TAG.Util.showLoading(displayLoading);
        ...
        for (var i = 0; i < relatedArts.length; i++) {
            TAG.Worktop.Database.getDoq(relatedArts[i], relatedArtHelperWrapper(i));
    ...
    */
    function makeRelatedArt(artInfo, tour, relatedTours) {
        //var xml = TAG.Worktop.Database.getDoqXML(artInfo.Identifier);
        ...
        art.css({
            ...
            'background-image': 'url(' + TAG.Worktop.Database.fixPath(artInfo.Metadata.Thumbnail) + ')',
        ...
        art.click(function () {
            ...
            curOpts.split = 'L';
            var deepZoom = new TAG.Layout.Artmode(prevPage, curOpts, tour);
            /* end nbowditch edit */
            root.css({ 'overflow-x': 'hidden' });
            TAG.Util.UI.slidePageLeftSplit(root, deepZoom.getRoot());
        });
    ...
    */
    function makeTour(tourInfo) {
        ...
        tour.css({
            ...
            'margin-right': '1%',
            'background-color': 'black',
            'background-image': 'url(' + TAG.Worktop.Database.fixPath(tourInfo.Metadata.Thumbnail) + ')',
            'background-size': 'contain',
            'background-repeat': 'no-repeat',
            'background-position': 'center center',
            overflow: 'hidden',
    ...
    */
    function switchPage(exhibition) {
        if (!switching) {
            switching = true;
            var catalog = new TAG.Layout.Catalog(exhibition);

            TAG.Util.UI.slidePageLeftSplit(root, catalog.getRoot(), function () {
    ...
    */
    function switchPageTour(tour) {
        ...
        if (!switching) {
            ...
            if (!rinData || !rinData.data) {
                var messageBox = $(TAG.Util.UI.popUpMessage(null, "Cannot play empty tour.", null));
            ...
            rinPlayer = new TAG.Layout.TourPlayer(rinData, tour);

            if (TAG.Util.Splitscreen.on()) {//if the splitscreen is on, exit it.
    ...
    function changeOrientation() {
        if (TAG.Util.Splitscreen.on()) { // Splitscreen and portrait mode
            leftbarHeader.css({
                height: '5%',
                width: '100%'
            });
...
TAG.Layout.Exhibitions.default_options = {};