...
LADS.TourAuthoring.Display = function (spec, my) {
    ...
    function initSVG() {
        ...
        $(fioD[0][0]).on("mousedown", function (e) {
            ...
            console.log("using new events");
            var offsetX = e.offsetX;
            _displayMousedown(offsetX - parseFloat(fioD.attr('x')), LADS.TourAuthoring.DisplayParts['fade-out']);
        });
                ...
                if (getEnd() - diff >= totalDispLength) {
                    if (fadeIn === 0 && fadeOut === 0) {
                        inStart = Math.constrain(diff,
                                        leftbound, // min
                                        Math.min(my.timeManager.pxToTime(parseFloat(fioHandle.css('left')) + 20 - 2 * LADS.TourAuthoring.Constants.fadeBtnSize), // max is one of two values
                ...
                if (diff + fadeOut - getStart() >= totalDispLength) {
                    if (fadeIn === 0 && fadeOut === 0) {
                        outStart = Math.constrain(diff,
                                        Math.max(my.timeManager.pxToTime(parseFloat(finHandle.css('left')) + 20) + my.timeManager.pxToTime(2 * LADS.TourAuthoring.Constants.fadeBtnSize),
