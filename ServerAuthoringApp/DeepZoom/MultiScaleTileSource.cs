    ...
    public abstract class MultiScaleTileSource : DependencyObject
    {
        ...
        internal IEnumerable<Tile> VisibleTilesUntilFill(Rect rectangle, int startingLevel)
        {
            ...
            while (level >= 0 && !isVisibleAreaFullyPainted)
            {
                ...
                visibleTiles = visibleTiles.Concat(tilesInLevel.Where(t => 1 == 1));
                level--;
            }*/
        ...
        internal int GetLevel(double scaleRatio)
        {
            ...
            return level.Clamp(0, ZoomLimitLevel);
        }
        ...
        internal int GetLevel(double viewportWidth, double viewportHeight)
        {
            ...
            {
                while (ImageSizeAtLevel(currentLevel).Height < viewportHeight && currentLevel < ZoomLimitLevel)
                    currentLevel++;
            }
            else
            {
                while (ImageSizeAtLevel(currentLevel).Width < viewportWidth && currentLevel < ZoomLimitLevel)
                    currentLevel++;
            }
            return currentLevel;
        }
        ...
        internal int TilesAtLevel(int level)
        {
            checked
            {
        ...
        protected internal int ZoomLimitLevel { get; private set; }
